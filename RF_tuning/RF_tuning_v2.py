...
TIME_BUDGET_GP = 2800
TIME_BUDGET_RF = 4
MAX_ITERATIONS_GP = 10
MAX_ITERATIONS_RF = 10
DIM = 1
REPETITIONS = 3
...
def random_forest_optimizer(function_name, function, values, max, number_of_trees, max_features, bootstrap, min_samples_split, run_no, rep_no):
    directory = "Testsv2/" + function_name + "/test" + str(run_no) + "/"
    if not os.path.exists(directory): os.makedirs(directory)

    json_dir = directory + function_name + "_scenario.json"
    csv_dir = directory + function_name + "_output_samples_" + str(rep_no) + ".csv"
...
def RF_hyperparameter_tuning(function):
    ...
    scenario["resume_optimization_data"] = "/mnt/d/Users/Luigi/Desktop/Universita/Tirocinio/RF_tuning/Optimizerv2/optimizer.csv"
...
def write_description_file(f_name, number_of_trees, max_features, bootstrap, min_samples_split, min, norm_min, run_no):
    dir = "Testsv2/" + f_name + "/tests_specs.csv"
